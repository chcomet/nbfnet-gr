...
def echarts(template_path, graph, title=None, node_colors=None, edge_colors=None, node_labels=None, relation_labels=None,
    ...
    with open(template_path, "r") as fin, open(save_file, "w") as fout:
...
def visualize_echarts(template_path, graph, sample, paths, weights, entity_vocab, relation_vocab, ranking=None, save_file=None,
    ...
    echarts(template_path, graph, title=title, node_colors=node_colors, node_labels=node_labels, relation_labels=relation_vocab,
...
if __name__ == "__main__":
    ...
    template_path = os.path.join(os.path.dirname(__file__), cfg.dataset.path, "echarts.html")
    ...
    for i in range(10):
        ...
        for j in range(solver.batch_size):
            ...
            if ranking[j, 0] <= 1000000 and not os.path.exists(save_file):
            ...
            if ranking[j, 1] <= 1000000 and not os.path.exists(save_file):
                paths, weights = task.visualize(sample)
                if paths:
